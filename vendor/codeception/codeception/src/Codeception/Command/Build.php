...
/**
 * Generates Actor model (initially Guy model) from suite configs.
 * Starting from Codeception 2.0 actor model are auto-generated. Use this command to generate them manually.
 *
 * * `codecept build`
...
class Build extends Command
{
    ...
    {
        return 'Generates base model for all suites';
    }
    ...
    private function buildSuiteActors()
    {
        ...
        if (!empty($suites)) {
            $this->output->writeln("<info>Building Actor model for suites: " . implode(', ', $suites) . '</info>');
        }
